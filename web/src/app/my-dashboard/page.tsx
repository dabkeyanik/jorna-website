...
export default function VendorDashboardPage() {
  ...
  const fetchAll = useCallback(async (): Promise<Snapshot | null> => {
    ...
    const [bookings, earnings, stripe, services, availability, reviews] =
      await Promise.all([
    ...
    return { vendor: me, bookings, earnings, stripe, services, availability, reviews };
  }, [user]);
            ...
            {requests.map((r) => (
              <Card key={r.booking_id} className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="serif text-lg text-ink">{r.client_name || "A client"}</p>
                    <p className="text-xs text-ink-faint">
                      {[r.event_name, prettyDate(r.date_iso)].filter(Boolean).join(" · ")}
                    </p>
                  </div>
          ...
          {availability.length === 0 ? (
            <div className="p-6 text-center">
              <p className="text-ink-soft">
                You haven&apos;t set any weekly hours. Hosts filtering by a date
                can&apos;t tell whether you&apos;re free.
              </p>
              <LinkButton href="/my-availability" className="mt-4">
                Set your hours
              </LinkButton>
            </div>
          ) : (
